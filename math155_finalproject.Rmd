...
The forecast R package has a function named forecast.Arima() which can be utilized to make predictions for upcoming values of a time series. Additionally, the level argument can be used to indicate the desired level of confidence for the prediction intervals.
```{r}
(fit <- Arima(train_ts, order=c(3,1,1)))
```
